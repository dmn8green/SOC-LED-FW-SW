        ...
        while (1) {
            ...
            // Data received
                ...
                if (strlen(rx_buffer) > 1) {
                    udp_server_callback(rx_buffer, len, tx_buffer, tx_buffer_len);
                }
...
void udp_server_callback(char *data, int len, char *out, int out_len) {
    ...
    if (error) {
        ...
        sprintf(out, "err:deserializeJson() failed: %s", error.c_str());
        return;
    }
    ...
    if (!root.containsKey("command")) {
        ...
        sprintf(out, "err:deserializeJson() failed: no cmd");
        return;
    }
    ...
    if (STR_IS_EQUAL(cmd, "get-chargepoint-config")) {
        ESP_LOGI(TAG, "get-chargepoint-config requested");
        handle_get_chargepoint_config(root, response);
    } else if (STR_IS_EQUAL(cmd, "set-chargepoint-config")) {
        ESP_LOGI(TAG, "set-chargepoint-config requested");
        handle_set_chargepoint_config(root, response);
    } else if (STR_IS_EQUAL(cmd, "unprovision-chargepoint")) {
        ESP_LOGI(TAG, "unprovision-chargepoint requested");
    ...
    auto reslen = serializeJson(docResponse, out, 8192);
    ESP_LOGI(TAG, "udp_server_callback: %s %d %d", out, out_len, reslen);
}
