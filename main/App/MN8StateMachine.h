...
typedef enum {
    e_mn8_unprovisioned,              //?! The device hasn't been provisioned with AWS and with the MN8 cloud.
...
protected:
    mn8_state_t unprovisioned_handle_event    (mn8_event_t event);
    mn8_state_t disconnected_handle_event     (mn8_event_t event);
    mn8_state_t disconnecting_handle_event    (mn8_event_t event);
    mn8_state_t connecting_handle_event       (mn8_event_t event);
    mn8_state_t connected_handle_event        (mn8_event_t event);
    mn8_state_t connection_error_handle_event (mn8_event_t event);

private:
    MN8Context* context = nullptr;

    mn8_state_t state = mn8_state_t::e_mn8_unprovisioned;
