...
static const char *TAG = "EthConfiguration";
 ...
 */
esp_err_t EthernetConfiguration::load() {
    KeyStore store;
    esp_err_t res;

    if (store.openKeyStore("eth_config", e_ro) != ESP_OK) {
...
esp_err_t EthernetConfiguration::save() {
    KeyStore store;
    esp_err_t res;

    if (store.openKeyStore("eth_config", e_rw) != ESP_OK) {
        ESP_LOGE(TAG, "Failed to open eth config store");
        return false;
    }
