...
void ChargingAnimation::reset(
    ...
    this->chasing_animation.reset(0x00000000, chase_color);
}
...
void ChargingAnimation::refresh(uint8_t* led_pixels, int led_count, int start_pixel) {
    ...
    this->static_charge_animation.refresh   (led_pixels, charging_pix_count, charged_led_count);
    this->static_remaining_animation.refresh(led_pixels, uncharged_led_count, charged_led_count + charging_pix_count);

    if (simulate_charge) {
        if ((++simulated_charge_percent) % 100 == 0) {
